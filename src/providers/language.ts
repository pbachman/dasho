...
export class LanguageProvider {
   ...
   * @param  {Events}           privateevents      Used to subscribe to the `user:language`
   * @param  {TranslateService} privatetranslate   The service from ng2-translate
   * @param  {Storage}          privatestorage     The storage from @ionic/storage
   */
  constructor(
   ...
   */
  public initialLanguage(): void {
    const DEFAULT_LANGUAGE = 'en';
    const BROWSER_LANG = this.translate.getBrowserLang();
    let choosenLanguage = BROWSER_LANG.match(/en|de/) ? BROWSER_LANG : DEFAULT_LANGUAGE;
   ...
   */
  public getTranslation(languageKey: string = this.translate.currentLang): any {
   ...
   */
  public get(key: string | Array<string>): string | Object {
   ...
   */
  public setLanguage(languageKey: string = this.translate.currentLang): void {
